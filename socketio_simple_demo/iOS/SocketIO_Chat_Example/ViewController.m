...
//        NSDictionary* config = @{@"log": @NO, @"compress": @YES, @"connectParams":@{@"EIO":@"3"}};
        NSDictionary *config = @{@"log": @YES, @"forcePolling": @YES};
        _sokManager = [[SocketManager alloc] initWithSocketURL:url config:config];
        _client = _sokManager.defaultSocket;
    }
    return _client;
}
...
- (void)connection{
    ...
    [self.client on:@"connect" callback:^(NSArray* data, SocketAckEmitter* ack) {
        NSLog(@"*************\n\niOS客户端上线\n\n*************");
        [self.client emit:@"login" with:@[@"30341"]];
    }];
...
- (IBAction)sendMessage:(id)sender {
    if (self.inputView.text.length>0) {
        ...
        [self.client emit:@"chat message" with:@[@{@"toUser":@"30342",@"message":self.inputView.text}]];
